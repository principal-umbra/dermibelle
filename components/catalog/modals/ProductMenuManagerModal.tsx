...
import { useProductMenuManager } from '../../../hooks/useProductMenuManager';
import ProductSectionSidebar from './product_menu/ProductSectionSidebar';
import ProductSectionEditor from './product_menu/ProductSectionEditor';
import ProductSectionPreview from './product_menu/ProductSectionPreview';
import { useData } from '../../../context/DataContext';

interface ProductMenuManagerModalProps {
    isOpen: boolean;
    onClose: () => void;
}

const ProductMenuManagerModal: React.FC<ProductMenuManagerModalProps> = ({ isOpen, onClose }) => {
    ...
    const {
        ...
        activeSection, linkedProducts, availableProducts,
        ...
        toggleProduct, addFeature, removeFeature, reorderSections
    } = useProductMenuManager();

    const [viewMode, setViewMode] = useState<'edit' | 'preview'>('edit');
    const [confirmAction, setConfirmAction] = useState<'publish' | 'unpublish' | null>(null);

    const handleSaveChanges = () => {
        addToast('success', 'Colección guardada correctamente.');
    };
    ...
    const confirmToggle = () => {
        if (confirmAction === 'publish') {
            ...
            addToast('success', 'Colección visible en tienda.');
        } else if (confirmAction === 'unpublish') {
            updateActiveSection('isActive', false);
            addToast('info', 'Colección oculta (Borrador).');
        }
                ...
                <div className="bg-white dark:bg-black/20 border-b border-gray-100 dark:border-gray-800 p-4 flex justify-between items-center shrink-0">
                    <div className="flex items-center gap-6">
                        <div>
                            <h2 className="text-xl font-display font-bold text-gray-900 dark:text-white flex items-center gap-2">
                                <span className="material-icons text-orange-500">storefront</span>
                                Gestor Tienda Online
                            </h2>
                            <p className="text-xs text-gray-500">Organiza tus productos para la venta al público.</p>
                        </div>
                    ...
                    <div className="flex gap-4 items-center">
                         ...
                         <button 
                            ...
                            className="bg-orange-500 hover:bg-orange-600 text-white px-5 py-2 rounded-xl shadow-lg shadow-orange-500/20 text-xs font-bold flex items-center gap-2"
                         >
                ...
                <div className="flex-1 flex overflow-hidden">
                    ...
                    {activeSection ? (
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center h-full text-gray-400 opacity-60 bg-gray-50 dark:bg-black/5">
                            ...
                            <p>Selecciona una colección para editar</p>
                        </div>
                    )}
                </div>
            </div>
...
export default ProductMenuManagerModal;
