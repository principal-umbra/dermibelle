    ...
    return (
        <div className="flex flex-col h-full bg-[#F3F4F6] dark:bg-background-dark p-4 md:p-6 overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 md:mb-6 gap-4 flex-shrink-0">
                <div>
                    <h1 className="text-xl md:text-2xl font-bold font-display text-gray-900 dark:text-white">Tablero de Citas</h1>
                    <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400">Gestión visual del flujo de trabajo.</p>
                </div>
                <div className="flex flex-wrap gap-2 md:gap-3">
                    <button onClick={archiveFinishedAppointments} className="flex-1 sm:flex-none px-3 md:px-4 py-2 bg-white border border-gray-200 text-gray-700 rounded-xl text-xs md:text-sm font-bold hover:bg-gray-50 flex items-center justify-center gap-2 shadow-sm"><span className="material-icons text-gray-400 text-base md:text-lg">inventory_2</span><span>Archivar</span></button>
                    <button onClick={() => setIsModalOpen(true)} className="flex-1 sm:flex-none px-4 md:px-5 py-2 bg-primary text-white rounded-xl text-xs md:text-sm font-bold shadow-lg shadow-primary/30 hover:bg-green-800 flex items-center justify-center gap-2"><span className="material-icons text-base md:text-lg">add</span><span>Nueva</span></button>
                </div>
            </div>

            <div className="flex flex-col sm:flex-row items-center justify-between bg-white p-2 rounded-xl shadow-sm border border-gray-100 mb-4 md:mb-6 gap-3 flex-shrink-0">
                <div className="flex items-center gap-3 md:gap-4 pl-1 md:pl-2">
                    <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">ORDEN:</span>
                    <div className="flex gap-2">
                        <span className="px-2 py-0.5 rounded-full bg-green-50 text-green-700 text-[10px] font-bold border border-green-100 cursor-pointer">Fecha Asc</span>
                        <span className="px-2 py-0.5 rounded-full text-gray-500 text-[10px] font-bold hover:bg-gray-50 cursor-pointer">Nuevas</span>
                    </div>
                </div>
                <div className="flex items-center gap-2 pr-1 md:pr-2">
                    <button className="flex items-center gap-1 text-[10px] font-bold text-gray-500 px-2 py-1 rounded-lg hover:bg-gray-50">Filtros <span className="material-icons text-sm">expand_more</span></button>
                </div>
            </div>

            <DragDropContext onDragEnd={onDragEnd}>
                <div className="flex-1 min-h-0 overflow-x-auto flex gap-4 md:gap-6 pb-4 custom-scrollbar">
                    {columns.map(col => (
                        <div key={col.id} className="w-[280px] md:w-80 flex-shrink-0 flex flex-col rounded-2xl h-full">
