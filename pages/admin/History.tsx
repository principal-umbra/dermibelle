            ...
            <div className="px-6 pt-6 pb-2 flex-shrink-0 z-20 relative">
                <div className="flex flex-wrap xl:items-center justify-between gap-4 md:gap-6 max-w-[1920px] mx-auto w-full">
                    ...
                    <div className="shrink-0 flex items-center gap-3 md:gap-4 pl-1 md:pl-2 w-full xl:w-auto">
                        ...
                        <div>
                            <h1 className="text-lg md:text-xl font-display font-bold text-gray-900 dark:text-white leading-tight">Centro Operativo</h1>
                            <p className="text-[10px] md:text-xs text-gray-500 dark:text-gray-400 font-medium tracking-wide">Gestión Integral</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 md:gap-4 overflow-x-auto no-scrollbar pb-2 md:pb-4 xl:pb-0 xl:justify-end flex-wrap flex-1 pl-1 md:pl-2 w-full xl:w-auto">
                        ...
                        <div className="flex items-center p-1 md:p-1.5 bg-white dark:bg-surface-dark/90 backdrop-blur-xl rounded-full border border-gray-200/60 dark:border-gray-700/60 shadow-lg md:shadow-xl shadow-gray-200/40 dark:shadow-black/40 ring-1 ring-white/50 dark:ring-white/5">
                            {visibleTabs.length === 0 ? (
                                <div className="px-4 md:px-6 py-2 text-xs md:text-sm text-gray-400 italic">Sin módulos</div>
                            ) : (
                                visibleTabs.map(tab => (
                                    <button
                                        key={tab.id}
                                    ...
                                    >
                                        <span className={`material-icons text-[16px] md:text-[18px] transition-transform duration-300 ${activeTab === tab.id ? '' : 'opacity-70 group-hover:scale-110'}`}>{tab.icon}</span>
                        ...
                        <button
                            ...
                            className={`p-2.5 md:p-3 rounded-full transition-all duration-300 shrink-0 border shadow-lg ${isSettingsOpen
                                ? 'bg-gray-900 text-white border-gray-900 rotate-90'
                                : 'bg-white dark:bg-surface-dark text-gray-400 border-gray-200 dark:border-gray-700 hover:text-primary hover:border-primary'}`}
                            title="Ajustes"
                        >
                            <span className="material-icons text-lg md:text-xl block">tune</span>
                        </button>
                    </div>
                </div>
            </div>
