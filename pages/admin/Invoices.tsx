...
const Invoices: React.FC = () => {
    ...
    return (
        <div className="flex flex-col h-full bg-[#F3F4F6] dark:bg-background-dark p-4 md:p-6 overflow-hidden">
            ...
            {/* Header */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4 md:mb-6 flex-shrink-0">
                <div>
                    <h1 className="font-display text-xl md:text-2xl font-bold text-gray-900 dark:text-white">Facturación</h1>
                    <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400">Control de ingresos y estados de cuenta.</p>
                </div>
                <div className="flex items-center gap-2 md:gap-3">
                    <div className="bg-white dark:bg-surface-dark px-2 md:px-3 py-1.5 md:py-2 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm flex items-center gap-2">
                        <span className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">Total</span>
                        <span className="text-xs md:text-sm font-display font-bold text-gray-900 dark:text-white">{invoices.length}</span>
                    </div>
                    <button onClick={() => setIsCreateModalOpen(true)} className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-3 md:px-4 py-2 bg-primary text-white rounded-lg hover:bg-green-800 transition-all shadow-md shadow-primary/20 text-xs md:text-sm font-bold hover:-translate-y-0.5 whitespace-nowrap">
                        <span className="material-icons text-base md:text-lg">add</span> Nueva
                    </button>
                </div>
            </div>
            ...
            <div className="flex-1 min-h-0 flex flex-col gap-4">
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 flex-shrink-0">
                    <div className="bg-white dark:bg-surface-dark rounded-xl p-3 md:p-4 border-l-4 border-green-500 shadow-sm flex items-center justify-between">
                        <div>
                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Pagado Mes</p>
                            <h3 className="text-sm md:text-xl font-display font-bold text-gray-900 dark:text-white mt-1">${totalPaidThisMonth.toFixed(0)}</h3>
                        </div>
                        <div className="hidden sm:flex w-8 h-8 md:w-10 md:h-10 rounded-full bg-green-50 text-green-600 items-center justify-center"><span className="material-icons text-lg md:text-xl">bar_chart</span></div>
                    </div>
                    <div className="bg-white dark:bg-surface-dark rounded-xl p-3 md:p-4 border-l-4 border-yellow-500 shadow-sm flex items-center justify-between">
                        <div>
                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Pendiente</p>
                            <h3 className="text-sm md:text-xl font-display font-bold text-gray-900 dark:text-white mt-1">${pendingAmount.toFixed(0)}</h3>
                        </div>
                        <div className="hidden sm:flex w-8 h-8 md:w-10 md:h-10 rounded-full bg-yellow-50 text-yellow-600 items-center justify-center"><span className="material-icons text-lg md:text-xl">pending_actions</span></div>
                    </div>
                    <div className="bg-white dark:bg-surface-dark rounded-xl p-3 md:p-4 border-l-4 border-indigo-500 shadow-sm flex items-center justify-between">
                        <div>
                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Cotizaciones</p>
                            <h3 className="text-sm md:text-xl font-display font-bold text-gray-900 dark:text-white mt-1">${quoteAmount.toFixed(0)}</h3>
                        </div>
                        <div className="hidden sm:flex w-8 h-8 md:w-10 md:h-10 rounded-full bg-indigo-50 text-indigo-600 items-center justify-center"><span className="material-icons text-lg md:text-xl">description</span></div>
                    </div>
                    <div className="bg-white dark:bg-surface-dark rounded-xl p-3 md:p-4 border-l-4 border-orange-500 shadow-sm flex items-center justify-between">
                        <div>
                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">En Tránsito</p>
                            <h3 className="text-sm md:text-xl font-display font-bold text-gray-900 dark:text-white mt-1">${inTransitAmount.toFixed(0)}</h3>
                        </div>
                        <div className="hidden sm:flex w-8 h-8 md:w-10 md:h-10 rounded-full bg-orange-50 text-orange-600 items-center justify-center"><span className="material-icons text-lg md:text-xl">sync_alt</span></div>
                    </div>
                </div>
                                        ...
                                        <tr key={inv.id} className="group hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                                            ...
                                            <td className="py-3 px-4 whitespace-nowrap">
                                                <button
                                                    ...
                                                    className={`font-mono font-bold text-sm px-2 py-0.5 rounded cursor-pointer hover:opacity-80 transition-all ${inv.status === 'Anulada' ? 'bg-gray-100 text-gray-400 line-through dark:bg-white/5' :
